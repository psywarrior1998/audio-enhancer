...
class AudioProcessorApp(ctk.CTk):
    ...
    def __init__(self, engine: AudioEngine):
        ...
        # --- Window & System Setup ---
        self.title("Aura Audio Suite v3.1 (Professional) | by Sanyam Sanjay Sharma")
    ...
    def _load_presets(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log.error(f"Failed to load presets.yaml: {e}", exc_info=True)
    ...
    def _save_presets(self):
        ...
        try:
            ...
            log.error(f"Failed to save presets: {e}", exc_info=True)
    ...
    def _create_waveform_display(self, parent: ctk.CTkFrame):
        ...
        self.ax.spines['top'].set_color('none'); self.ax.spines['right'].set_color('none')
        fig.tight_layout()
    ...
    def _create_preset_controls(self, parent: ctk.CTkFrame):
        ...
        self.preset_menu = ctk.CTkOptionMenu(preset_frame, variable=self.preset_var, values=list(self.presets.keys()) or ["No Presets"], command=self.load_preset)
    ...
    def _create_separation_controls(self, parent: ctk.CTkFrame):
        ...
        ctk.CTkCheckBox(parent, text="AI Vocal Separation", variable=self.sep_var).pack(anchor='w', pady=(15, 5), padx=10)
        self.sep_model_display_var = ctk.StringVar(value=list(self.sep_model_map.keys())[0])
        ctk.CTkOptionMenu(parent, variable=self.sep_model_display_var, values=list(self.sep_model_map.keys())).pack(anchor='w', padx=30, pady=5, fill='x')

    def _create_eq_controls(self, parent: ctk.CTkFrame):
        self.eq_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(parent, text="3-Band Parametric EQ", variable=self.eq_var).pack(anchor='w', pady=(15, 5), padx=10)
    ...
    def _create_dynamics_controls(self, parent: ctk.CTkFrame):
        ...
        ctk.CTkCheckBox(parent, text="Noise Gate", variable=self.gate_var).pack(anchor='w', pady=(15, 5), padx=10)
        self.compress_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(parent, text="Dynamic Compression", variable=self.compress_var).pack(anchor='w', pady=5, padx=10)

    def _create_batch_tab(self, tab: ctk.CTkFrame):
        tab.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(tab, text="Apply current settings from the 'Processing' tab to all audio files in a folder.", wraplength=400, justify="left").pack(fill='x', pady=10, padx=10)
        ...
        ctk.CTkButton(tab, text="Start Batch Process", command=self.start_batch_thread).pack(pady=20, fill='x', padx=10)
    ...
    def _create_settings_tab(self, tab: ctk.CTkFrame):
        ...
        if not self.is_cuda_available:
            ...
            ctk.CTkLabel(tab, text="NVIDIA GPU not detected. CPU will be used.", font=ctk.CTkFont(size=12)).pack(anchor='w', padx=10)
    ...
    def draw_waveform(self, path: str):
        try:
            ...
            self.ax.set_xlabel(''); self.ax.set_ylabel(''); self.ax.set_yticks([])
            self.canvas.draw()
            self.update_status("Status: Idle")
        except Exception as e:
            self.update_status(f"Status: Error loading waveform")
            log.error(f"Could not load waveform for {path}: {e}", exc_info=True)
            messagebox.showerror("Waveform Error", f"Could not load waveform:\n{e}")

    def _get_processing_options(self) -> Dict[str, Any]:
        return {'use_separation': self.sep_var.get(), 'separation_model': self.sep_model_map[self.sep_model_display_var.get()],
    ...
    def save_preset(self):
        ...
        if preset_name:
            if preset_name in self.presets and not messagebox.askyesno("Confirm Overwrite", f"Preset '{preset_name}' already exists. Overwrite?"):
                return
    ...
    def load_preset(self, preset_name: str):
        ...
        self.gate_var.set(settings.get('use_gate', True)); self.compress_var.set(settings.get('use_compression', True))
        self.use_cuda_var.set(settings.get('use_cuda', self.is_cuda_available))
        self.update_status(f"Status: Preset '{preset_name}' loaded.")
        log.info(f"Loaded preset: {preset_name}")

    def delete_preset(self):
        preset_name = self.preset_var.get()
        if not preset_name or preset_name in ["Select Preset...", "No Presets"]:
        ...
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the preset '{preset_name}'?"):
            if preset_name in self.presets:
                ...
                self.preset_menu.configure(values=list(self.presets.keys()) or ["No Presets"])
                self.update_status(f"Status: Preset '{preset_name}' deleted.")
                log.info(f"Deleted preset: {preset_name}")
    ...
    def start_processing_thread(self, preview: bool = False, file_path: Optional[str] = None):
        ...
        thread = threading.Thread(target=self.process_audio, args=(path, self._get_processing_options(), preview), daemon=True)
        thread.start()
    ...
    def start_batch_thread(self):
        ...
        self.is_processing = True
        self.toggle_ui_state(tk.DISABLED)
        thread = threading.Thread(target=self.process_batch, args=(folder_path, self._get_processing_options()), daemon=True)
        thread.start()

    def progress_callback(self, event_type: str, *args: Any):
        if event_type == "status": self.update_status(f"Status: {args[0]}", args[1] if len(args) > 1 else None)
        elif event_type == "progress": self.progress.set(args[0]/100.0)
        elif event_type == "error": messagebox.showerror("Processing Error", f"An error occurred: {args[0]}")
        elif event_type == "success": messagebox.showinfo("Success", args[0])
    ...
    def process_audio(self, path: str, options: Dict[str, Any], preview: bool):
        ...
        try:
            ...
            self.is_processing = False; self.toggle_ui_state(tk.NORMAL); self.update_status("Status: Idle", 0)
    ...
    def process_batch(self, folder_path: str, options: Dict[str, Any]):
        ...
        if not audio_files:
            ...
            self.is_processing = False; self.toggle_ui_state(tk.NORMAL); return
        total_files = len(audio_files)
        log.info(f"Starting batch process for {total_files} files in {folder_path}")
        for i, filename in enumerate(audio_files):
            self.progress_callback("status", f"Batch processing {i+1}/{total_files}: {filename}", int((i/total_files)*100))
            file_path = os.path.join(folder_path, filename)
            try:
                self.engine.run_pipeline(file_path, options, lambda *args: None) # Use a dummy callback for batch
            except Exception as e:
                log.error(f"Failed to process batch file {filename}: {e}", exc_info=True)
        self.progress_callback("status", "Batch process complete!", 100); self.is_processing = False; self.toggle_ui_state(tk.NORMAL)
        log.info("Batch process complete.")

    def update_status(self, text: str, progress: Optional[float] = None):
        self.status_label.configure(text=text)
        if progress is not None: self.progress.set(progress / 100.0 if progress > 1 else progress)

    def toggle_ui_state(self, state: str):
        self.preview_button.configure(state=state); self.process_button.configure(state=state)
        self.preset_menu.configure(state=state)

    def _on_closing(self):
        if self.is_processing and not messagebox.askyesno("Exit", "A process is still running. Are you sure you want to exit?"):
            return
...
if __name__ == "__main__":
    try:
        ...
        log.critical(f"Fatal Error: config.yaml not found. {e}", exc_info=True)
        messagebox.showerror("Fatal Error", "config.yaml not found. Please ensure it is in the same directory as the application.")
    except Exception as e:
