...
# It's crucial to import these from the engine file
...
class AudioProcessorApp(ctk.CTk):
    """
    Final version of the professional audio processing GUI with a correctly
    synchronized visual progress bar and cancellable operations.
    """
    ...
    def __init__(self, engine: AudioEngine):
        ...
        self.title("Aura Audio Suite v3.7 (Professional) | by Sanyam Sanjay Sharma")
        ...
        try:
            if preview:
                ...
                if not stop_event.is_set():
                    ...
                    self.playback_thread = threading.Thread(target=play, args=(processed_snippet,), daemon=True); self.playback_thread.start()
            else:
            ...
            self.status_label.configure(text="Status: Process Cancelled")
            self.progress.set(0)
            ...
            if not self.stop_event.is_set():
                self.status_label.configure(text="Status: Idle"); self.progress.set(0)
    ...
    def toggle_ui_state(self, processing: bool):
        if processing:
            ...
            state = "normal"; self.stop_button.grid_remove();
            self.preview_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew")
            self.process_button.grid(row=2, column=1, padx=10, pady=10, sticky="ew")
    ...
    def _load_presets(self):
        ...
        try:
            with open(self.presets_path, 'r') as f:
                ...
                log.info(f"Loaded {len(presets)} presets from {self.presets_path}"); return presets
    ...
    def process_batch(self, folder_path: str, options: Dict[str, Any], stop_event: threading.Event):
        ...
        try:
            ...
            if not stop_event.is_set():
                self.status_label.configure(text="Batch process complete!"); self.progress.set(1)
