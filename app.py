...
class AudioProcessorApp(ctk.CTk):
    """
    Final version with dynamic resizing, parallel processing, and all features.
    """
    ...
    def __init__(self, engine: AudioEngine):
        ...
        self.title("Aura Audio Suite v4.1 (Parallel Engine) | by Sanyam Sanjay Sharma")
    ...
    def _create_widgets(self):
        # --- MODIFICATION: Configure grid weights for dynamic resizing ---
    ...
    def _create_status_display(self, parent: ctk.CTkFrame):
        status_frame = ctk.CTkFrame(parent); status_frame.grid(row=1, column=0, sticky="ew", pady=(20, 0)); status_frame.grid_columnconfigure((0, 1), weight=1)
        self.status_label = ctk.CTkLabel(status_frame, text="Status: Idle", anchor="w", justify="left"); self.status_label.grid(row=0, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
        self.progress = ctk.CTkProgressBar(status_frame, mode='determinate'); self.progress.set(0); self.progress.grid(row=1, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="ew")
        ...
        self.preview_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew"); self.process_button.grid(row=2, column=1, padx=10, pady=10, sticky="ew")

    def _get_processing_options(self) -> Dict[str, Any]:
        return {
            'use_separation': self.sep_var.get(),'separation_model': self.sep_model_map[self.sep_model_display_var.get()],
            'use_eq': self.eq_var.get(),'eq_low': self.eq_low_var.get(),'eq_mid': self.eq_mid_var.get(),'eq_high': self.eq_high_var.get(),
            'use_gate': self.gate_var.get(),'gate_threshold': self.config['defaults']['gate_threshold_dbfs'],
            'use_compression': self.compress_var.get(),'use_cuda': self.use_cuda_var.get(),
            'low_ram_mode': self.low_ram_var.get(),'trim_silence': self.trim_silence_var.get(),
            'output_format': self.output_format_var.get(),'parallel_processing': self.parallel_processing_var.get()
        }
    ...
    def _create_processing_chain_tab(self, tab: ctk.CTkFrame):
        ...
        self.trim_silence_var = ctk.BooleanVar(value=False); ctk.CTkCheckBox(tab, text="Trim Long Silences", variable=self.trim_silence_var).pack(anchor='w', pady=(15, 5), padx=10)
        output_frame = ctk.CTkFrame(tab, fg_color="transparent"); output_frame.pack(fill='x', pady=10, padx=10)
        ctk.CTkLabel(output_frame, text="Output Format:").pack(side="left"); self.output_format_var = ctk.StringVar(value="wav")
        ctk.CTkOptionMenu(output_frame, variable=self.output_format_var, values=["wav", "flac"], width=200).pack(side="left", padx=10)
    ...
    def _create_settings_tab(self, tab: ctk.CTkFrame):
        ...
        gpu_checkbox = ctk.CTkCheckBox(tab, text="Use GPU (NVIDIA CUDA)", variable=self.use_cuda_var); gpu_checkbox.pack(anchor='w', pady=10, padx=10)
        if not self.is_cuda_available: gpu_checkbox.configure(state='disabled'); ctk.CTkLabel(tab, text="NVIDIA GPU not detected.", font=ctk.CTkFont(size=12)).pack(anchor='w', padx=10)
        self.low_ram_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(tab, text="Low RAM Mode (Slower, for < 8GB RAM)", variable=self.low_ram_var).pack(anchor='w', pady=10, padx=10)
        self.parallel_processing_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(tab, text="Enable Parallel Processing (For long audio files)", variable=self.parallel_processing_var).pack(anchor='w', pady=10, padx=10)
    
    # All other methods (event handlers, helpers, etc.) are included below for completeness
    def progress_callback(self, event_type: str, *args: Any):
        if event_type == "status": self.status_label.configure(text=args[0]);
        if len(args) > 1: self.progress.set(args[1] / 100.0)
        elif event_type == "progress": self.progress.set(args[0] / 100.0)
        elif event_type == "error": messagebox.showerror("Processing Error", f"An error occurred: {args[0]}"); self.status_label.configure(text="Status: Error"); self.progress.set(0)
        elif event_type == "success":
            if not self.stop_event.is_set(): messagebox.showinfo("Success", args[0])
            self.status_label.configure(text="Status: Complete!"); self.progress.set(1)
    def stop_processing(self):
        if self.is_processing: log.warning("Stop button clicked."); self.stop_event.set(); self.stop_button.configure(state="disabled", text="Stopping...")
    def start_processing_thread(self, preview: bool = False, file_path: Optional[str] = None):
        ...
        self.stop_event.clear(); self.is_processing = True; self.toggle_ui_state(processing=True)
        thread = threading.Thread(target=self.process_audio, args=(path, self._get_processing_options(), preview, self.stop_event), daemon=True); thread.start()
        ...
        try:
            if preview:
                ...
                if not stop_event.is_set():
                    self.progress_callback("status", "Playing preview...", 0); self.playback_thread = threading.Thread(target=play, args=(processed_snippet,), daemon=True); self.playback_thread.start()
            else:
            ...
            log.info("Process was cancelled by the user."); self.status_label.configure(text="Status: Process Cancelled"); self.progress.set(0)
        except Exception as e:
            log.error(f"Error during processing: {e}", exc_info=True); self.progress_callback("error", str(e))
        finally:
            ...
            self.is_processing = False; self.toggle_ui_state(processing=False)
            if not self.stop_event.is_set(): self.status_label.configure(text="Status: Idle"); self.progress.set(0)
    def toggle_ui_state(self, processing: bool):
        if processing: state = "disabled"; self.preview_button.grid_remove(); self.process_button.grid_remove(); self.stop_button.grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
        else: state = "normal"; self.stop_button.grid_remove(); self.preview_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew"); self.process_button.grid(row=2, column=1, padx=10, pady=10, sticky="ew"); self.stop_button.configure(state="normal", text="Stop Process")
        self.preset_menu.configure(state=state)
    def _load_presets(self):
        if not os.path.exists(self.presets_path): return {}
        try:
            with open(self.presets_path, 'r') as f: presets = yaml.safe_load(f) or {}; log.info(f"Loaded {len(presets)} presets"); return presets
        except (yaml.YAMLError, IOError) as e: log.error(f"Failed to load presets: {e}", exc_info=True); messagebox.showerror("Preset Load Error", f"Failed to load: {e}"); return {}
    def _save_presets(self):
        try:
            with open(self.presets_path, 'w') as f: yaml.dump(self.presets, f, default_flow_style=False, indent=2); log.info(f"Saved {len(self.presets)} presets")
        except IOError as e: log.error(f"Failed to save presets: {e}", exc_info=True); messagebox.showerror("Preset Save Error", f"Failed to save: {e}")
    ...
    def draw_waveform(self, path: str):
        try:
            self.status_label.configure(text="Status: Loading waveform..."); self.progress.set(0); y, sr = librosa.load(path, sr=None, mono=True); self.ax.clear()
            librosa.display.waveshow(y, sr=sr, ax=self.ax, color='#00AFFF'); self.ax.set_xlabel(''); self.ax.set_ylabel(''); self.ax.set_yticks([])
            self.canvas.draw(); self.status_label.configure(text="Status: Idle")
        except Exception as e:
            self.status_label.configure(text="Status: Error loading waveform"); log.error(f"Could not load waveform for {path}: {e}", exc_info=True); messagebox.showerror("Waveform Error", f"Could not load waveform:\n{e}")
    def save_preset(self):
        ...
        if preset_name:
            ...
            self.status_label.configure(text=f"Status: Preset '{preset_name}' saved."); log.info(f"Saved preset: {preset_name}")
    def load_preset(self, preset_name: str):
        ...
        self.low_ram_var.set(settings.get('low_ram_mode', False)); self.trim_silence_var.set(settings.get('trim_silence', False)); self.output_format_var.set(settings.get('output_format', 'wav'))
        self.parallel_processing_var.set(settings.get('parallel_processing', True))
        self.status_label.configure(text=f"Status: Preset '{preset_name}' loaded."); log.info(f"Loaded preset: {preset_name}")
    def delete_preset(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the preset '{preset_name}'?"):
            if preset_name in self.presets:
                ...
                self.preset_menu.configure(values=list(self.presets.keys()) or ["No Presets"]); self.status_label.configure(text=f"Status: Preset '{preset_name}' deleted."); log.info(f"Deleted preset: {preset_name}")
    def start_batch_thread(self):
        ...
        if self.is_processing: messagebox.showwarning("Busy", "Another process is already running."); return
        self.stop_event.clear(); self.is_processing = True; self.toggle_ui_state(processing=True)
        thread = threading.Thread(target=self.process_batch, args=(folder_path, self._get_processing_options(), self.stop_event), daemon=True); thread.start()
    def process_batch(self, folder_path: str, options: Dict[str, Any], stop_event: threading.Event):
        ...
        try:
            for i, filename in enumerate(audio_files):
                ...
                self.status_label.configure(text=f"Batch: {i+1}/{total_files}: {filename}"); self.progress.set(progress_percent / 100.0)
                file_path = os.path.join(folder_path, filename)
                self.engine.run_pipeline(file_path, options, lambda *args: None, stop_event)
        except UserCancelledError:
            log.info("Batch process was cancelled by the user."); self.status_label.configure(text="Status: Batch Cancelled"); self.progress.set(0)
        except Exception as e:
             log.error(f"Failed to process batch file {filename}: {e}", exc_info=True); messagebox.showerror("Batch Error", f"An error occurred on {filename}:\n{e}")
        finally:
            self.is_processing = False; self.toggle_ui_state(processing=False)
            if not stop_event.is_set(): self.status_label.configure(text="Batch process complete!"); self.progress.set(1)
